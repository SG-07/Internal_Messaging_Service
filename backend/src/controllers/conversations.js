...
function buildWorkflow(conversation, currentUserId) {
  const { category, workflow_status, created_by, workflow_comment } =
    conversation;
  ...
  const finalStatuses =
    type === "action" ? FINAL_ACTION_STATUSES : FINAL_APPROVAL_STATUSES;
...
async function fetchUserProfile(
  userId,
  selectFields = "id, username, full_name, email",
) {
  return supabaseAdmin
    .from("profiles")
    .select(selectFields)
...
async function fetchMessages(conversationId) {
  return supabaseAdmin
    .from("messages")
    .select(
      `id, content, created_at, sender_id, profiles:sender_id(id, username, full_name)`,
    )
    .eq("conversation_id", conversationId)
...
function formatConversationMeta(conv, senderProfile, otherProfile) {
  return {
    ...
    other_user_name:
      otherProfile?.profiles?.full_name ||
...
async function validateWorkflowTransition(category, currentStatus, newStatus) {
  ...
  const validStatuses = isAction
    ? Object.keys(ACTION_TRANSITIONS).concat(
        Object.values(ACTION_TRANSITIONS).flat(),
      )
    : Object.keys(APPROVAL_TRANSITIONS).concat(
        Object.values(APPROVAL_TRANSITIONS).flat(),
      );
  ...
  try {
    ...
    const formattedConversations = await Promise.all(
      conversationLinks.map(async (cp) => {
        ...
        const { data: otherParticipant } = await fetchOtherParticipant(
          conv.id,
          user_id,
        );
...
export const getConversation = async (req, res) => {
  ...
  try {
    // Verify participant
    const { error: participantError } = await verifyParticipant(
      conversationId,
      user_id,
    );
    if (participantError) {
    ...
    const { data: conversation, error: convError } =
      await fetchConversationFull(conversationId);
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to fetch conversation." });
  }
};
...
export const updateConversation = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to update conversation." });
  }
};
...
export const deleteConversation = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to remove conversation." });
  }
};
...
export const addMessage = async (req, res) => {
  ...
  try {
    ...
    // Fetch sender profile
    const { data: senderProfile } = await fetchUserProfile(
      sender_id,
      "username, full_name",
    );
...
export const getConversationsWithUser = async (req, res) => {
  ...
  if (!email) {
    return res
      .status(400)
      .json({ success: false, message: "Provide an email to search." });
  }
  ...
  try {
    ...
    if (myConvIds.length === 0) {
      return res
        .status(200)
        .json({ success: true, message: "No conversations found.", data: [] });
    }
  ...
  try {
    ...
    const formattedConversations = await Promise.all(
      sentConversations.map(async (conv) => {
        const { data: recipient } = await fetchOtherParticipant(
          conv.id,
          user_id,
        );
        return {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to fetch sent conversations." });
  }
};
...
export const getPendingWorkflows = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to fetch pending items." });
  }
};
...
export const getMyWorkflowRequests = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to fetch your requests." });
  }
};
...
export const updateActionStatus = async (req, res) => {
  ...
  try {
    ...
    logIfDev("[updateActionStatus]", {
      conversationId,
      from: conversation.workflow_status,
      to: status,
    });
    ...
    res.status(200).json({
      ...
      workflow: {
        ...
        workflow_comment: updated.workflow_comment,
        is_final: isFinal,
        can_respond: !isFinal, // the actor calling this endpoint is always the assignee, never the creator (already enforced above)
      },
    });
  } catch (err) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to update action status." });
  }
};
...
export const updateApprovalStatus = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Unable to update approval status." });
  }
};
...
export const reviewReport = async (req, res) => {
  ...
  try {
    ...
    if (status === "resolved" || status === "dismissed") {
      const { data: otherPendingReports, error: checkError } =
        await supabaseAdmin
          .from("conversation_reports")
          .select("id")
      ...
      if (
        !checkError &&
        (!otherPendingReports || otherPendingReports.length === 0)
      ) {
        ...
        if (markClearError) {
          console.error(
            "[reviewReport] Error clearing conversation report flag:",
            markClearError,
          );
...
export const listReports = async (req, res) => {
  ...
  try {
    ...
    // Build query
    let query = supabaseAdmin.from("conversation_reports").select(
      `
        id,
        conversation_id,
        reported_by,
    ...
    if (status) {
      ...
      if (!validStatuses.includes(status)) {
        return res.status(400).json({
          success: false,
          message:
            "Invalid status. Must be one of: pending, reviewed, dismissed, resolved.",
        });
      }
...
export const reportConversation = async (req, res) => {
  ...
  try {
    ...
    if (conversation.is_group) {
      ...
      const { data: groupMembership, error: groupMemberError } =
        await supabaseAdmin
          .from("group_members")
          .select("id")
    ...
    if (existingReportError) {
      console.error(
        "[reportConversation] Error checking existing report:",
        existingReportError,
      );
    ...
    if (existingReport) {
      if (existingReport.status === "pending") {
        return res.status(400).json({
          success: false,
          message:
            "You have already reported this conversation. Your report is pending review.",
        });
      }
      if (existingReport.status === "resolved") {
        return res.status(400).json({
          success: false,
          message:
            "You have already reported this conversation and it has been resolved.",
        });
      }
    ...
    if (markReportedError) {
      console.error(
        "[reportConversation] Error marking conversation as reported:",
        markReportedError,
      );
    ...
    if (isDev) {
      console.log(
        "[reportConversation] Conversation reported:",
        conversationId,
        "by user:",
        user_id,
        "report_id:",
    ...
    res.status(201).json({
      success: true,
      message:
        "Thank you for reporting this conversation. Our team will review it shortly.",
      data: {
        report_id: report.id,
        conversation_id: conversationId,
        conversation_subject: conversation.subject,
