...
const ACTION_STATUSES = {
  ...
  MORE_INFO: {
    ...
    description: "More information is required before the action can proceed.",
  },
...
function ActionSection({
  ...
  const currentStatus = ACTION_STATUSES[status] || ACTION_STATUSES.PENDING;
  ...
  async function handleStatusChange() {
    ...
    try {
      ...
      await onStatusChange(selectedStatus, trimmedComment);

      setSelectedStatus(null);
      setComment("");
    } catch (err) {
      setError(
        err.message || "Unable to update the action status. Please try again.",
      );
    } finally {
      setUpdating(false);
    }
  ...
  return (
    <section className="mx-6 mb-6 rounded-xl border border-amber-200 bg-amber-50 p-5">
        ...
        {canRespond && !selectedStatus && (
          <div className="flex flex-wrap gap-2">
            <button
              type="button"
              disabled={updating}
              onClick={() => openStatusForm("WILL_DO")}
      ...
      {updating && (
        <p className="mt-3 text-xs text-gray-500">Updating action status...</p>
      )}
